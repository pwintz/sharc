...
def compute_metrics(exp_dir):
    ...
    target_speed = controller_params.get("TerminalVelocity", sp.get("target_speed", None))
    target_units = str(
        controller_params.get(
            "TerminalVelocityUnits",
            sp.get("target_speed_units", "mps" if cfg.get("dynamics_class_name") == "CarlaMPCDynamics" else "kmph"),
        )
    ).lower() if target_speed is not None else None
    target_speed_kph = None if target_speed is None else (
        float(target_speed) * 3.6 if target_units in ("mps", "m/s") else float(target_speed)
    )
    speed_to_kph = 3.6 if cfg.get("dynamics_class_name") == "CarlaMPCDynamics" else 1.0
    ...
    if target_speed_kph is not None and x_steps.shape[1] >= 4:
        speed = x_steps[:, 3] * speed_to_kph
        speed_errors = speed - target_speed_kph
    ...
    if x_steps.shape[1] >= 4 and n_steps > 0:
        metrics["final_speed"] = float(x_steps[-1, 3] * speed_to_kph)
    else:
