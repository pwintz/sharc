...
//   if (debug_interfile_communication_level >= 1) 
//   {
//     PRINT_WITH_FILE_LOCATION("Sending " << label << " (k=" << i_loop << "): \"" << out_str << "\" to Python");
//   } 
//   if (!outfile.is_open()) {
//     throw std::runtime_error("Error: Output file is not open or in a bad state!");
//   } else if (!outfile.good()) {
//     throw std::runtime_error("Error: Output file is not open or in a bad state!");
//   } else {
//     PRINT_WITH_FILE_LOCATION("Pipe file for " << label << " is OK.");
//   }
//   auto out_str = x.transpose().format(fmt_high_precision);
//   outfile << "Loop " << i_loop << ": " << out_str << std::endl;
...
//   if (debug_interfile_communication_level >= 1) 
//   {
//     PRINT_WITH_FILE_LOCATION(label << " (sent to Python): " << out_str);
//   }
// }
// 
// void sendDouble(std::string label, int i_loop, double x, std::ofstream& outfile) 
...
// 
// template<int rows>
// void readCVec(std::string filename, std::ifstream& in_file, mpc::cvec<rows>& x) {
// 
//     // Check debug level and print the appropriate message
//     if (debug_interfile_communication_level >= 2) {
//         PRINT_WITH_FILE_LOCATION("Reading a vector from \"" << filename << "\".");
//     }
// 
//     // Get the line from the file
//     std::string x_in_line_from_py;
//     std::getline(in_file, x_in_line_from_py);
//     if (x_in_line_from_py.starts_with("ERROR")){
//       PRINT_WITH_FILE_LOCATION("Reading " + filename + " failed because it contained an error message. The message was " + x_in_line_from_py)
//       throw std::runtime_error("Reading " + filename + " failed because it contained an error message. The message was " + x_in_line_from_py);
//     }
// 
//     if (debug_interfile_communication_level >= 2) {
...
//   bool use_state_after_delay_prediction = json_data["system_parameters"]["mpc_options"]["use_state_after_delay_prediction"];
// 
//   // ------ Start of controller definition ------ 
// 
//   // Vector sizes.
//   const int Tnx = 5;  // State dimension
//   const int Tnu = 1;  // Control dimension
...
//   const int control_horizon = CONTROL_HORIZON;
// 
//   // Check that PREDICTION_HORIZON during compilation match those passed in config JSON.
//   if (prediction_horizon != json_data["system_parameters"]["mpc_options"]["prediction_horizon"]) {
//     throw  std::runtime_error("prediction_horizon (compiled) does not match the value in the JSON.");
//   }
//   if (control_horizon != json_data["system_parameters"]["mpc_options"]["control_horizon"]) {
...
//     readCVec(x_in_filename, x_infile, modelX);
//     PRINT("x=" << modelX.transpose().format(fmt));
//     PRINT("u=" << u.transpose().format(fmt));
// 
//     // Begin a batch of Scarab statistics
...
// 
//     if (!use_external_dynamics_computation) {
//       // If use internal dynamics computation
//       throw std::runtime_error("Internal dynamics are no longer suppoer");
//       // printVector("modelX", modelX);
//       // printMat("optseq.state", optseq.state);
//       // modelX = Ad * modelX + Bd * u + Bd_disturbance*lead_car_input;
//       // printVector("modelX", modelX);
